  ...
  return (
    <section className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div>
        ...
        <p className="text-sm text-slate-500">Collaborate with the Arc-i-Tech delivery squad.</p>
      </div>
      <div
        className={`flex flex-1 flex-col gap-3 overflow-y-auto rounded-xl bg-slate-50 p-4 ${compact ? 'max-h-72' : 'max-h-96'}`}
      >
        {messages.length === 0 && (
          <p className="text-sm text-slate-500">Your conversation will appear here. Drop a message to get started.</p>
        )}
        {messages.map((message) => {
          const isAgent = message.senderRole !== 'CUSTOMER';
          return (
            <div key={message.id} className={`flex flex-col ${isAgent ? 'items-start' : 'items-end'}`}>
              <div className={`rounded-2xl px-4 py-2 text-sm leading-relaxed shadow-sm ${isAgent ? 'bg-white text-slate-700' : 'bg-indigo-600 text-white'}`}>
                <p className="font-semibold">{isAgent ? message.senderName : 'You'}</p>
                <p className="mt-1">{message.message}</p>
              </div>
              <span className="mt-1 text-xs text-slate-400">{formatRelative(message.sentAt)}</span>
            </div>
          );
        })}
      </div>
